...
function gfs1p0degPath(attr, type, surface, level) {
  ...
  var file =
    [stamp, type, surface, level, 'gfs', '1.0'].filter(µ.isValue).join('-') +
    '.json'
...
function gfsDate(attr) {
  if (attr.date === 'current') {
    ...
    var now = new Date(Date.now()),
      hour = Math.floor(now.getUTCHours() / 3)
    return new Date(
  ...
  relative_humidity: {
    ...
    create: function (attr) {
      return buildProduct({
        ...
        paths: [
          gfs1p0degPath(attr, 'relative_humidity', attr.surface, attr.level),
        ],
        date: gfsDate(attr),
        builder: function (file) {
          ...
          var rh =
            ...
            vars.Relative_humidity_height_above_ground
          var data = rh.data
          return {
            header: netcdfHeader(
              vars.time,
              vars.lat,
              vars.lon,
              file.Originating_or_generating_Center
            ),
            interpolate: bilinearInterpolateScalar,
            data: function (i) {
              return data[i]
    ...
    create: function (attr) {
      ...
      return buildProduct({
        ...
        builder: function (windFile, airdensFile) {
          ...
          var windData = windBuilder.data,
            windInterpolate = windBuilder.interpolate
          var airdensData = airdensBuilder.data,
            airdensInterpolate = airdensBuilder.interpolate
          return {
