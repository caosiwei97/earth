...
function parse(hash, projectionNames, overlayTypes) {
  var option,
    result = {}
  //             1        2        3          4          5            6      7      8    9
  var tokens = /^(current|(\d{4})\/(\d{1,2})\/(\d{1,2})\/(\d{3,4})Z)\/(\w+)\/(\w+)\/(\w+)([\/].+)?/.exec(
    hash
  )
  if (tokens) {
    var date =
...
/**
 * Backbone.js Model层，它将其属性保存为一个人性化的URL哈希片段。
 *
 * 加载和存储到哈希片段是由sync方法处理的。
 *
...
/**
 * Backbone模型将页面配置作为一组属性保存，例如日期、层、投影、方向等
 * 对配置的更改将触发页面组件对事件的响应
 * 例如：配置configuration.save({projection: "orthographic"})的修改将会导致地球以正射投影重新渲染
 *
 * 返回Configuration实例对象，它代表哈希片段，使用被允许的投影。
 *
