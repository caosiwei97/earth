...
var configuration = µ.buildConfiguration(globes, products.overlayTypes) // holds the page's current configuration settings  保存页面的当前配置设置
...
function drawOverlay(field, overlayType) {
  ...
  if (grid) {
    ...
    colorBar.on('mousemove', function () {
      ...
      var elementId =
        grid.type === 'wind' ? '#location-wind-units' : '#location-value-units'
...
function init() {
  ...
  // 定义画布大小为浏览器的视口宽高（clientWidth，clientHeight）
  d3.selectAll('.fill-screen')
    .attr('width', view.width)
    .attr('height', view.height)

  // 调整菜单作为scale的canvas元素填充右边宽度
